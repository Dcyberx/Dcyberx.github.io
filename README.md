...
<head>
  ...
  <style>
    ...
    .ascii-content{
      ...
      left:0;
      right:0;
      animation:asciiScroll 20s linear infinite;
    }
  ...
  <div class="container">
    <main>
      ...
      <section class="card meta">
        ...
        <p style="margin:8px 0 0;color:var(--muted)">I build software with security and scale in mind. Best colour: blue. Profile image: <code>me.png</code>. Logo file: <code>cyber.png</code></p>
        <div class="sections">
          <div>
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
  ...
  <script>
    ...
    (function(){
      ...
      function computeArea(){ const r = canvas.getBoundingClientRect(); return {width:r.width,height:r.height}; }
      function initColumns(){
        const a = computeArea(); cols.length = 0;
        for(let i=0;i<COLUMN_COUNT;i++){
          const x = Math.floor((a.width / COLUMN_COUNT) * i + (Math.random()*8));
          const speed = MIN_SPEED + Math.random()*(MAX_SPEED - MIN_SPEED);
          const length = Math.floor(MIN_LENGTH + Math.random()*(MAX_LENGTH - MIN_LENGTH));
          const delay = Math.random()*2000;
          const el = document.createElement('div');
          el.className='column'; el.style.left=x+'px'; el.style.fontSize=FONT_SIZE+'px';
          el.style.opacity=(0.6+Math.random()*0.45).toString();
          canvas.appendChild(el);
          cols.push({el,x,speed,length,chars:[],y:-Math.random()*a.height-20,lastUpdate:performance.now(),delay});
        }
      }
      function update(now){
        const a = computeArea();
        for(const c of cols){
          ...
          const dt=(now-c.lastUpdate)/1000; c.lastUpdate=now; c.y+=c.speed*dt;
          if(Math.random()<0.02) c.speed*=(0.95+Math.random()*0.1);
          if(c.y-(c.length*FONT_SIZE)>a.height){
            c.y=-Math.random()*100; c.length=Math.floor(MIN_LENGTH+Math.random()*(MAX_LENGTH - MIN_LENGTH));
          ...
          for(let i=0;i<c.length;i++){
            ...
            if(i===c.length-1) chars.push(`<span class="lead">${ch}</span>`);
            else chars.push(`<span style="opacity:${Math.max(0.15,(i/c.length)*0.9)}">${ch}</span>`);
          }
      ...
      function refreshBinscroll(){
        let s=''; for(let r=0;r<80;r++){for(let k=0;k<6;k++)s+=CHAR_SET[Math.random()>.5?0:1];s+='\n';}
        binscroll.textContent=s;
      }
      function start(){
        canvas.innerHTML=''; initColumns(); refreshBinscroll();
        requestAnimationFrame(ts=>{cols.forEach(c=>c.lastUpdate=ts); update(ts);});
        setInterval(refreshBinscroll,1200);
        window.addEventListener('resize',()=>{canvas.innerHTML=''; initColumns();});
      }
      start();
    })();
    ...
    window.addEventListener('load', () => {
      try {
        ...
        msg.rate = 1.55; msg.pitch = 0.65; msg.volume = 1;
